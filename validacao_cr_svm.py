...
print("Iniciando a validação cruzada SVM...")
...
print("\n--- Resultados Finais da Validação Cruzada (SVM) ---")
...
if len(y_true_all) > 0 and len(set(y_true_all)) == 2:
    ...
    plt.title('Curva ROC da Validação Cruzada - SVM')
    plt.legend(loc="lower right")
    plt.grid(True)
    plt.show()
else:
    print("\nCurva ROC/AUC não disponível para problemas multiclasse ou sem probabilidades.")