...
feature_names = list(previsores.columns)
...
print("Iniciando a validação cruzada para XGBoost...")
...
print("\n--- Resultados Finais da Validação Cruzada (XGBoost) ---")
...
if len(y_true_all) > 0 and len(set(y_true_all)) == 2:
    ...
    plt.title('Curva ROC da Validação Cruzada - XGBoost')
    plt.legend(loc="lower right")
    plt.grid(True)
    plt.show()
    
# Importância das características (Top 30 com Porcentagens)
print("\nTreinando o modelo final com todos os dados...")


# Instancia o classificador final com os mesmos hiperparâmetros
classificador_final = XGBClassifier(
    n_estimators=1000,
    max_depth=7,
    learning_rate=0.05,
    gamma=0.1,
...
classificador_final.fit(previsores, classe)
print("\nGerando gráfico de importância das features...")
importances = classificador_final.feature_importances_
...
bars = ax.barh(range(top_n), top_importances[::-1], align='center')
...
ax.set_xlim(right=top_importances.max() * 1.18) 
